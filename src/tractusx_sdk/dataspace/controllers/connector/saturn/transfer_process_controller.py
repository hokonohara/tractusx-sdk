...
from tractusx_sdk.dataspace.models.connector.saturn import TransferProcessModel


class TransferProcessController(StatefulEntityDmaController, DmaController):
    """
    Concrete implementation of the TransferProcessController for the Connector saturn Data Management API.
    ...
    def resume_by_id(self, oid: str, **kwargs):
        return self.adapter.post(url=f"{self.endpoint_url}/{oid}/resume", **kwargs)
    
    def suspend_by_id(self, oid: str, **kwargs):
        return self.adapter.post(url=f"{self.endpoint_url}/{oid}/suspend", **kwargs)
